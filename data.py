...
def create_dataframe(file:str) -> pd.DataFrame:
    ...
    :return: DataFrame
    """
    data_frame = pd.read_csv(file, names=['re_path','abs_path'])
    return data_frame
...
def stat(dataframe:pd.DataFrame)-> None:
    """
    Printing all stats
    :param dataframe: DataFrame
    :return: None
    """
    print(dataframe[['Height','Width','Channels']].describe())
...
def sort(dataframe: pd.DataFrame,max_h:int,max_w:int)->pd.DataFrame:
    ...
    """
    new_data = dataframe.copy(deep=True)[(dataframe['Height']<max_h)&(dataframe['Width']<max_w)]
    return new_data
