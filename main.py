    ...
    def select_csv(self) -> None:
        ...
        if filename:
            if os.stat(filename).st_size == 0:
                QMessageBox.warning(self, "Ошибка", "Выбранный файл пуст. Пожалуйста, выберите другой файл.")
                return
            try:
                self.path = filename
                ...
                print(f"Произошла ошибка: {str(v)}")
                QMessageBox.warning(self, "Ошибка", str(v))
            except Exception as e:
                print(f"Произошла ошибка: {str(e)}")
                QMessageBox.critical(self, "Ошибка", f"Произошла ошибка при чтении файла: {str(e)}")
